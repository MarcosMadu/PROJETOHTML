...
// Geração de PDF com layout de tabela e cores estilizadas
...
app.get('/notificacoes/:id/pdf', async (req, res) => {
  try {
    ...
    // Headers para forçar download como PDF
    ...
    // 1️⃣ Primeira página: dados principais
    // Título grande
    ...
    // 2️⃣ Segunda página: EVIDÊNCIAS FOTOGRÁFICAS
    doc.addPage();
...
app.post(
  '/inspecao',
  upload.any(),           // Multer aceita qualquer campo de arquivo
  async (req, res) => {
    try {
      // Monta um objeto com as respostas:
      const respostas = {};
      for (let i = 1; i <= 19; i++) {
        respostas[`resposta${i}`] = req.body[`resposta${i}`] || '';
      }

      // Monta um mapa de arrays de fotos:
      const fotos = {};
      (req.files || []).forEach(f => {
